...
ParticleSystem::ParticleSystem(const glm::vec3 &position, Geometry *geom, Shader *shader, GLuint texture, GameObject *parent)
    ...
    explosion_life_time_ = 0.0;
    explosion_life_end_time_ = 3.0;
...
void ParticleSystem::Update(double delta_time) {
    ...
    //explosion life time handle
    if (GetState() == GameObject::Alive && parent_->GetState() == GameObject::Exploded) {
        //increase life time & check limit
        explosion_life_time_ += delta_time;
        if (explosion_life_time_ >= explosion_life_end_time_) {
            //it died
            state_ = Dead;
        }
    }
    else if (GetState() == GameObject::Alive && parent_->GetState() == GameObject::Dead) {
        state_ = Dead;
    }
